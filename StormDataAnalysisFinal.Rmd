...
This R markdown file analyzes Storm Data from U.S.National Oceanic and Atmospheric Administration's (NOAA) database to answer below some basic questions about severe weather events,
...
Step1: Download and unzip strom data stored in bz2 file to csv file.  Renamed the file name to StormData.csv 

Step2: We will read content of this file as a raw data for our analysis. We will load this data into variable StormData to make our data accessibility easier
...
Reading Data:
We first read data from the comma-separated-value file for data analysis i.e. StormData.csv file.
...
Event <- with(StormData, aggregate(INJURIES + FATALITIES ~ EVTYPE, data=StormData, FUN = "sum"))
...
For data processing we will use StormData variable from above which has all required dataset.
...
SubsetData <- subset(StormData, (StormData$CROPDMGEXP == "M" | StormData$CROPDMGEXP == "K" | StormData$CROPDMGEXP == "B"  | StormData$CROPDMGEXP == "m" | StormData$CROPDMGEXP == "k" | StormData$CROPDMGEXP == "b" | StormData$CROPDMGEXP == "h" | StormData$CROPDMGEXP == "H") | (StormData$PROPDMGEXP =="M" | StormData$PROPDMGEXP =="K" | StormData$PROPDMGEXP =="B" | StormData$PROPDMGEXP =="m" | StormData$PROPDMGEXP =="k" | StormData$PROPDMGEXP =="b" | StormData$PROPDMGEXP =="h" | StormData$PROPDMGEXP =="H") )
...
for(i in 1:length(SubsetData$CROPDMGEXP)){
                ...
                ifelse(SubsetData$CROPDMGEXP[i] == "B" | SubsetData$CROPDMGEXP[i] == "b", SubsetData$CROPDMG[i] <- SubsetData$CROPDMG[i] * 1000000000, 
                ifelse(SubsetData$CROPDMGEXP[i] == "H" | SubsetData$CROPDMGEXP[i] == "h", SubsetData$CROPDMG[i] <- SubsetData$CROPDMG[i] * 100, SubsetData$CROPDMG[i] <- SubsetData$CROPDMG[i] ))))
  }
...
for(i in 1:length(SubsetData$PROPDMGEXP)) {
                ...
                ifelse(SubsetData$PROPDMGEXP[i] == "B" | SubsetData$PROPDMGEXP[i] == "b", SubsetData$PROPDMG[i] <- SubsetData$PROPDMG[i] * 1000000000, 
                ifelse(SubsetData$PROPDMGEXP[i] == "H" | SubsetData$PROPDMGEXP[i] == "h", SubsetData$PROPDMG[i] <- SubsetData$PROPDMG[i] * 100, SubsetData$PROPDMG[i] <- SubsetData$PROPDMG[i] ))))
  }
...
EconomicDamage <- with(SubsetData, aggregate(CROPDMG + PROPDMG ~ EVTYPE, data=StormData, FUN = "sum"))
